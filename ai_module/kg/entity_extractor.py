...
Input chính:
    - doc.authors:   list[str] raw  → AuthorEntity      (regex)
    - doc.sections:  list[Section]  → ConceptEntity      (LLM)
    - doc.tables:    list[Table]    → EvidenceEntity     (LLM)
    - doc.figures:   list[Figure]   → EvidenceEntity     (LLM)
    - doc.abstract:  str            → ConceptEntity/FindingEntity fallback
    - doc.keywords:  list[str]      → ConceptEntity(category="task") fallback
    - doc.language:  str            → điều chỉnh prompt language

Entity schema (khớp với entities.py):
    AuthorEntity    — tác giả
    ConceptEntity   — method / model / task / theory / ...  (category phân biệt)
    EvidenceEntity  — dataset / benchmark / survey / ...
    MetricEntity    — F1, Accuracy, p-value, ...
    FindingEntity   — kết luận / claim / contribution
    PaperEntity     — được build từ doc metadata, không extract bằng LLM

LLM backend:
    AnthropicBackend — production (ANTHROPIC_API_KEY)
    OllamaBackend    — local/dev

Changelog:
    v5 — 2025-05 (performance optimization)
        [v5-1]  Gộp 4 prompt riêng lẻ → 1 prompt _PROMPT_COMBINED per chunk
                Giảm ~75% số LLM call (từ 4×N → 1×N calls)
        [v5-2]  Parallel sections bằng ThreadPoolExecutor
                Các section được xử lý song song, không tuần tự
        [v5-3]  Giới hạn _MAX_SECTIONS_PER_DOC = 8 (priority sections ưu tiên)
                Tránh xử lý toàn bộ 11+ sections cho paper xã hội/kinh tế
        [v5-4]  _parse_json_object() riêng cho combined response (dict thay vì list)
        [v5-5]  Table/figure captions gộp vào 1 LLM call thay vì 2 call riêng
        [v5-6]  max_workers=4 configurable qua __init__

    v5.1 — 2025-05 (fix JSON parse error)
        [v5.1-1] Tách _PROMPT_COMBINED thành header (có {placeholders}) + schema tĩnh
                 (không có placeholders) để tránh double-brace bug khi dùng str.format().
                 Root cause: Python str.format() yêu cầu escape {{ }} cho literal braces,
                 nhưng LLM nhìn thấy {{ trong prompt rồi reproduce thành { { trong output,
                 gây JSON parse error 100% sections.
        [v5.1-2] Cập nhật category list trong schema prompt khớp với CONCEPT_CATEGORIES v3
                 (thêm regulation, project, contract, financial_instrument,
                  technology, process, material, system)
        [v5.1-3] Cập nhật evidence_type list trong schema prompt khớp với EVIDENCE_TYPES v3
                 (thêm book, standard, event)

    v5.2 — 2025-05 (fix data quality)
        [v5.2-1] Thêm _repair_double_brace() — fix Qwen2.5 sinh [{ { obj } }] thay vì [{ obj }]
                 Áp dụng cho cả _parse_json_object và _parse_json_list
                 2-attempt strategy: thử parse thẳng trước, chỉ repair nếu fail
        [v5.2-2] Thêm _resolve_category() và _resolve_evidence_type() —
                 normalize pipe-separated value: "method|algorithm" → "method"
        [v5.2-3] Filter category="person" trong build_entities — skip entity tên người
                 LLM hay nhầm tên tác giả thành ConceptEntity
        [v5.2-4] Redirect evidence_type thuộc về Concept → skip EvidenceEntity
                 Dùng EVIDENCE_TYPE_REDIRECT_TO_CONCEPT từ entities.py
                 VD: evidence_type="organization" → skip (nên là ConceptEntity)
                 Thêm _FIGURE_TABLE_RE vào _is_noise_evidence —
                 filter "hình 4", "table_1" không phải evidence thật

    v4 — 2025-05 (sync với entities.py v2)
        [v4-1..13] — xem changelog gốc
"""
