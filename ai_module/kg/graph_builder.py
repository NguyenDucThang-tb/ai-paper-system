...
Nhận UnifiedDocument + ExtractedEntities + RelationResult → build Knowledge Graph Neo4j.

Thứ tự giai đoạn cứng:
    Giai đoạn 1: Paper → Author + Institution → Venue → Topic → Citation stubs
    Giai đoạn 2: Concept/Evidence/Metric nodes + edges từ RelationResult
    Giai đoạn 3: Chunk nodes (sau khi Qdrant đã upsert)

Edge type được build (khớp relation_extractor.py v3):
    WROTE, PUBLISHED_AT, HAS_TOPIC, CITES          — Phase 1
    USES_CONCEPT, EVALUATES_ON, ACHIEVES_METRIC    — Phase 2
    BASED_ON, EXTENDS                               — Phase 2
    SUPPORTS, CONTRADICTS                           — Phase 2 (từ FindingEntity)
    HAS_CHUNK                                       — Phase 3

Changelog:
    v3 — 2025-05 (sync với entities.py v2 + relation_extractor.py v3)
        [v3-1]  Import đổi: bỏ MethodEntity/DatasetEntity/TaskEntity
                → ConceptEntity/EvidenceEntity/MetricEntity/FindingEntity
        [v3-2]  Import RelationResult: BasedOnRelation.child_method → child_concept
        [v3-3]  build_phase2: entities.methods/datasets/tasks → concepts/evidences/metrics
        [v3-4]  build_phase2: merge_method/dataset/task → merge_concept/evidence/metric
        [v3-5]  build_phase2: thêm _insert_metric_edges, _insert_extends_edges,
                _insert_supports_edges, _insert_contradicts_edges
        [v3-6]  _merge_based_on: child_method/parent_method → child_concept/parent_concept
        [v3-7]  _insert_edges_from_relations: uses_method → uses_concept,
                addresses_task bị xóa, thêm achieves_metric/extends/supports/contradicts
        [v3-8]  _merge_paper: thêm domain từ PaperEntity nếu có

    v3.1 — 2025-05
        [v3.1-1] _merge_concept_hierarchy: thêm in-memory cycle detection (DFS) trước
                 khi gọi Neo4j, tránh tạo cycle trong BASED_ON/EXTENDS graph.
                 Cypher guard trong neo4j_client vẫn giữ như lớp 2 defense.

    v2 — 2025-05
        [fix-1..10] — xem changelog gốc
"""
