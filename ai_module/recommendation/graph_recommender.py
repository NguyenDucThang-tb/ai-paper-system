...
5 tín hiệu chính (có thể bật/tắt từng cái qua GraphRecommenderConfig):
    ...
    5. AUTHOR_NETWORK    — cùng Author (WRITTEN_BY) hoặc cùng Institution  [v1.5]
...
Usage:
    ...
    for r in results:
        print(r.paper_id, r.title, r.score, r.breakdown)
...
Changelog:
    ...
    v1.3 — 2025-05
        [v1.3-1] GraphRecommendation: thêm field title: str = ""
        [v1.3-2] _enrich_shared: thêm query_title batch lấy title cho tất cả target_ids

    v1.4 — 2025-05  Bug fixes:
        [fix-7] batch_recommend: sửa logic insert_similar_to
        [fix-8] min_score filter: đổi <= thành <
        [fix-9] _signal_concept_hierarchy: deduplicate (other_id, hop)
        [fix-10] GraphRecommenderConfig.__post_init__: revalidate tổng weight sau reset
        [fix-11] _enrich_shared: sửa comment — 3 queries chạy tuần tự

    v1.5 — 2025-05  Thêm Signal 5 — Author/Institution Network:
        [v1.5-1] GraphRecommenderConfig: thêm weight_author_network=0.15
                 Các weight cũ scale xuống tương ứng để tổng vẫn = 1.0:
                   concept 0.40→0.35, evidence 0.25→0.20, citation 0.20→0.15, hierarchy 0.15→0.15
                 Thêm author_institution_weight: float = 0.6
                   — shared institution yếu hơn shared author (1.0)
                 Thêm author_concept_filter_method: bool = True
                   — chỉ boost author score nếu paper cùng author cũng dùng
                     ít nhất 1 concept category="method" giống nhau
        [v1.5-2] _signal_author_network: query mới
                 2 sub-signal:
                   a) shared_author  — cùng node Author (WRITTEN_BY)         score × 1.0
                   b) shared_institution — cùng author.affiliation            score × 0.6
                 Nếu author_concept_filter_method=True:
                   chỉ tính paper có ít nhất 1 concept category="method" chung
                   với paper nguồn (JOIN thêm USES_CONCEPT → Concept{category:"method"})
        [v1.5-3] recommend(): thêm author_scores vào pipeline chuẩn
                 norm_author tích hợp đầy đủ như 4 signal cũ
        [v1.5-4] _enrich_shared: shared_authors giờ được populate thực sự
                 (trước đây luôn trả [] — chỉ tính ở "phase1 nếu cần")
                 Thêm query_author batch lấy shared author names cho target_ids
        [v1.5-5] GraphRecommendation.breakdown: thêm key "author_network"
"""
