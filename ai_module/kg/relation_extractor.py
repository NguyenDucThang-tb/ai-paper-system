...
Edge type đầy đủ (khớp entities.py v2):
    ...
    Giai đoạn 2 — hybrid (rule detect, LLM verify):
        (Paper)   -[:USES_CONCEPT   {category}]->  (Concept)   ← thay USES_METHOD
        (Paper)   -[:EVALUATES_ON             ]->  (Evidence)  ← thay EVALUATES_ON
        (Paper)   -[:ACHIEVES_METRIC          ]->  (Metric)    ← NEW
        (Concept) -[:BASED_ON                 ]->  (Concept)
        (Concept) -[:EXTENDS                  ]->  (Concept)   ← NEW

    Giai đoạn 3 — từ FindingEntity (không cần LLM):
        (Paper)   -[:SUPPORTS                 ]->  (Paper)     ← NEW
        (Paper)   -[:CONTRADICTS              ]->  (Paper)     ← NEW

Changelog:
    v3 — 2025-05 (sync với entities.py v2)
        [v3-1]  Bỏ MethodEntity/DatasetEntity/TaskEntity — đổi sang ConceptEntity/EvidenceEntity
        [v3-2]  USES_METHOD → USES_CONCEPT, thêm field category vào edge
        [v3-3]  Thêm AchievesMetricRelation — (Paper)-[:ACHIEVES_METRIC]->(Metric)
        [v3-4]  Thêm ExtendsRelation — (Concept)-[:EXTENDS]->(Concept)
        [v3-5]  Thêm SupportsRelation + ContradictsFindingRelation từ FindingEntity
        [v3-6]  EvidenceEntity không có field metric — dùng MetricEntity thay thế
        [v3-7]  _score_concept_relation thay _score_method_relation
        [v3-8]  _score_evidence_relation thay _score_dataset_relation
        [v3-9]  entities.concepts/evidences/metrics/findings thay methods/datasets/tasks
        [v3-10] LLM verify prompt cập nhật USES_CONCEPT thay USES_METHOD
        [v3-11] _merge_verified cập nhật dedup theo ConceptEntity/EvidenceEntity

    v2 — 2025-05
        [fix-1..7] — xem changelog gốc
"""
