"""
ai_module/retrieval/graph_retriever.py

Query Knowledge Graph (user_doc_kg) để lấy context hỗ trợ RAG.

Input:
    - query_text : câu hỏi của user
    - paper_id   : paper đang được hỏi (optional — nếu có thì filter theo paper)
    - top_k      : số lượng kết quả tối đa

Output: GraphRetrievalResult
    - chunks      : list[GraphChunk]   — dạng text, tương thích với vector_retriever
    - subgraph    : SubgraphContext     — dạng structured (nodes + edges)
    - query_info  : QueryInfo           — debug / trace

Luồng:
    user query
        ↓
    _extract_query_entities()   — tách entity từ query (keyword match + Neo4j fulltext)
        ↓
    _retrieve_by_entities()     — query Neo4j theo entity → subgraph
    _retrieve_by_paper()        — query Neo4j theo paper_id → paper context
    _retrieve_by_concept_chain()— BASED_ON / EXTENDS chain (graph traversal)
        ↓
    _build_text_chunks()        — convert subgraph → text chunks cho fusion_retriever
        ↓
    GraphRetrievalResult

Design decisions:
    - Dùng cùng Neo4jClient từ storage/graph_db/neo4j_client.py
    - Database name truyền vào qua GraphRetrieverConfig — mặc định "user_doc_kg"
    - Không dùng LLM ở đây (retrieval thuần graph)
    - Score được tính từ: entity match + section confidence + concept confidence
    - Tất cả Cypher query đều có LIMIT để tránh timeout trên graph lớn

Tương thích với fusion_retriever.py:
    GraphChunk có đủ fields: text, score, source, metadata
    → fusion_retriever có thể merge với VectorChunk không cần adapter

Changelog:
    v1   — initial implementation
    v1.1 — thêm _retrieve_by_concept_chain() cho BASED_ON / EXTENDS traversal
    v1.2 — thêm SubgraphContext.to_cypher_summary() cho context_builder
    v1.3 — thêm GraphRetrieverConfig, tách database config khỏi hardcode
"""
